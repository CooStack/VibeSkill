...
遇到强调“不要”、纠正上一轮实现或用具体对象举例时，按 [否定反馈与举例](references/message-workflow.md#否定反馈与举例) 更新当前任务规格，区分目标、有效约束、纠偏历史和示例用途。命名与注释描述当前职责；禁令继续约束实现，示例按用途提取规范或保留精确内容。减少无关信息进入产物，不降低约束优先级，也不从否定自行推断替代需求。
